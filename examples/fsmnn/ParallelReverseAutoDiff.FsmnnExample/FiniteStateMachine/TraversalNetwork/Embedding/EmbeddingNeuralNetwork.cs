...
namespace ParallelReverseAutoDiff.FsmnnExample.FiniteStateMachine.TraversalNetwork.Embedding
{
    ...
    public class EmbeddingNeuralNetwork : NeuralNetwork
    {
        ...
        public async Task<DeepMatrix> AutomaticBackwardPropagate(DeepMatrix gradient)
        {
            ...
            backwardStartOperation = this.computationGraph["row_concatenate_trans_0_0"];
        ...
        public void InitializeState()
        {
            if (this.Output == null)
            {
                this.Output = new Matrix(CommonMatrixUtils.InitializeZeroMatrix((this.NumIndices * this.EmbeddingSize) + 3, 1).ToArray());
            }
            else
            {
                this.Output.Replace(CommonMatrixUtils.InitializeZeroMatrix((this.NumIndices * this.EmbeddingSize) + 3, 1).ToArray());
            }

            if (this.Input == null)
            {
                this.Input = new Matrix(CommonMatrixUtils.InitializeZeroMatrix(this.NumIndices, 1).ToArray());
            }
            else
            {
                this.Input.Replace(CommonMatrixUtils.InitializeZeroMatrix(this.NumIndices, 1).ToArray());
            }
        }
