//------------------------------------------------------------------------------
// <copyright file="CategoricalVarianceLossOperation.cs" author="ameritusweb" date="9/4/2023">
...
namespace ParallelReverseAutoDiff.FsmnnExample.FiniteStateMachine.RMAD
{
    ...
    /// </summary>
    public class CategoricalVarianceLossOperation
    {
        private const double Lambda = 100d;
        private const double Alpha = 0.5d;
        private const double Beta = 0.5d;
        ...
        public double Forward(Matrix predicted, int targetIndex, double trueVariance)
        {
        ...
        public Matrix Backward()
        {
            ...
            {
                dOutput[0][i] = (Alpha * (Lambda * ((4.0 / this.predicted.Cols) *
                    (this.variance - this.trueVariance) *
                    (this.predicted[0][i] - this.mean)))) + (Beta * (this.targetIndex == i ? -1 / this.predicted[0][i] : 0d));
            }
