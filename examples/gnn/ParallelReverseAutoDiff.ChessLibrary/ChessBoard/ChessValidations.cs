    ...
    public partial class ChessBoard
    {
        ...
        internal static bool IsValidMove(Move move, ChessBoard board, bool raise, bool checkTurn)
        {
            ...
            else
            {
                if (isValid && raise)
                {
                    board.OnInvalidMoveKingCheckedEvent(new CheckEventArgs(board, move.Piece?.Color == PieceColor.White ? board.WhiteKing.GetValueOrDefault() : board.BlackKing.GetValueOrDefault(), true));
                }
                return false;
            }
        }
        ...
        internal static bool HasRightToCastle(PieceColor side, CastleType castleType, ChessBoard board)
        {
            ...
            bool ValidByMoves()
            {
                Position kingpos = new((short)4, (short)(side == PieceColor.White ? 0 : 7)); // "e1" : "e8"

                var rookpos = castleType switch
                {
                    CastleType.King => new Position((short)7, (short)(side == PieceColor.White ? 0 : 7)), // "h1" : "h8"
                    CastleType.Queen => new Position((short)0, (short)(side == PieceColor.White ? 0 : 7)), // "a1" : "a8"
