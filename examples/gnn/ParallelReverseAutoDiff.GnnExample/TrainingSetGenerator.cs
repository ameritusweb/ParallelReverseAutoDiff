...
namespace ParallelReverseAutoDiff.GnnExample
{
    ...
    public class TrainingSetGenerator
    {
        ...
        public void AddToBag(ConcurrentBag<GapGraph> bagOfGraphs, string fen, Move targetMove, Move lastMove)
        {
            bagOfGraphs.Add(this.ProcessFen(fen, targetMove, lastMove));
        }
        ...
        private GapGraph ProcessFen(string fen, Move targetMove, Move lastMove)
        {
            this.gameState = new GameState();
            this.gameState.Board = ChessBoard.LoadFromFen(fen);
            return this.ConstructGraph(targetMove, lastMove, fen);
        }
        ...
        private List<GapGraph> ProcessMoves(List<Move> moves, string name, bool shouldSave)
        {
            ...
            try
            {
                foreach ((Move move, Move? nextmove) in moves.WithNext())
                {
                    ...
                    if (nextmove != null)
                    {
                        GapGraph graph = this.ConstructGraph(nextmove, this.gameState.Board.ExecutedMoves.Last());
        ...
        private GapGraph ConstructGraph(Move nextmove, Move lastMove, string? fenString = null)
        {
            ...
            var fen = fenString ?? this.gameState.Board.ToFen();
            ...
            graph.UpdateFeatureIndices(this.artifacts, this.gameState.Board.ToPositionFen(), this.gameState.Board.ExecutedMoves.Any() ? this.gameState.Board.ExecutedMoves.Last().ToString() : lastMove.ToString(), gamePhase);
