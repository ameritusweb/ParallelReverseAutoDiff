...
namespace ParallelReverseAutoDiff.VGruExample.VGruNetwork.RMAD
{
    ...
    public class SquaredArclengthEuclideanMagnitudeLossOperation
    {
        ...
        public Matrix Forward(Matrix predictions, double targetAngle, double targetMagnitude)
        {
            ...
            var dRadius_dXOutput = xOutput / radius;
            var dRadius_dYOutput = yOutput / radius;
            ...
            var dNormalizedDotProduct_dXOutput = ((xTarget * radius * radius) - (2 * xOutput * dotProduct)) / Math.Pow(radius, 4);
            var dNormalizedDotProduct_dYOutput = ((yTarget * radius * radius) - (2 * yOutput * dotProduct)) / Math.Pow(radius, 4);
            ...
            // h'(x) = 2 * f(x) * g(x) * (f'(x)g(x) + f(x)g'(x))
            var dArcLength_dXOutput = 2 * radius * theta * ((dRadius_dXOutput * theta) + (radius * dTheta_dXOutput));
            var dArcLength_dYOutput = 2 * radius * theta * ((dRadius_dYOutput * theta) + (radius * dTheta_dYOutput));
