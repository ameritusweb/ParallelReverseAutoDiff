//------------------------------------------------------------------------------
// <copyright file="DynamicGradientClipper.cs" author="ameritusweb" date="5/7/2023">
...
namespace ParallelReverseAutoDiff.VGruExample
{
    ...
    /// A gradient clipper.
    /// </summary>
    public class DynamicGradientClipper
    {
        private readonly NeuralNetwork network;

        /// <summary>
        /// Initializes a new instance of the <see cref="DynamicGradientClipper"/> class.
        /// </summary>
        /// <param name="network">The neural network.</param>
        public DynamicGradientClipper(NeuralNetwork network)
        {
        ...
        private Matrix ClipGradients(Matrix gradients, double clipValue, double? minValue)
        {
            ...
            var standardizedMatrix = this.StandardizedMatrix(gradients, clipValue);
            ...
            for (int i = 0; i < numRows; i++)
            {
                for (int j = 0; j < numCols; j++)
                {
                    var value = Math.Min(clipValue, Math.Abs(standardizedMatrix[i][j]));
        ...
        private Matrix StandardizedMatrix(Matrix matrix, double clipValue)
        {
            // Calculate the mean
            double sum = 0;
            int count = 0;
            ...
            double maxWithDev = max / stdDev;

            double multiplier = maxWithDev >= clipValue ? 1d : clipValue / maxWithDev;
            ...
            for (int i = 0; i < matrix.Length; i++)
            {
                ...
                {
                    standardizedMatrix[i][j] = matrix[i][j] / stdDev * multiplier;
                }
            }
