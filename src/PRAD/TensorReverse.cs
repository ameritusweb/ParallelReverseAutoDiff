...
namespace ParallelReverseAutoDiff.PRAD
{
    ...
    public class TensorReverse
    {
        ...
        public Tensor SumReverse(Tensor upstreamGradient, int[] axes)
        {
            ...
            Tensor result = new Tensor(originalShape, 0.0);
        ...
        public Tensor ExcludeReverse(Tensor upstreamGradient, double min, double max)
        {
            ...
            var gradData = new double[tensor.Data.Length];
        ...
        public Tensor ClipReverse(Tensor upstreamGradient, double min, double max)
        {
            ...
            var gradData = new double[tensor.Data.Length];
            var minArray = new double[tensor.Data.Length];
            var maxArray = new double[tensor.Data.Length];
            var zeroArray = new double[tensor.Data.Length];

            // Fill minArray, maxArray and zeroArray with appropriate values
            Array.Fill(minArray, min);
            Array.Fill(maxArray, max);
            Array.Fill(zeroArray, 0.0);

            // Create intermediate arrays for comparison
            var clippedMin = new double[tensor.Data.Length];
            var clippedMax = new double[tensor.Data.Length];
            var clippedMask = new double[tensor.Data.Length];
            ...
            Parallel.For(0, tensor.Data.Length, i =>
            {
                clippedMask[i] = (clippedMax[i] == tensor.Data[i]) ? 1.0 : 0.0;
            });
        ...
        public Tensor MeanReverse(Tensor upstreamGradient, int axis)
        {
            ...
            double[] gradData = new double[inputTensor.Data.Length];

            double[] scaleData = new double[innerSize];
            ...
            Parallel.For(0, outerSize, outerIdx =>
            {
                ...
                double[] scaledUpstream = new double[innerSize];
