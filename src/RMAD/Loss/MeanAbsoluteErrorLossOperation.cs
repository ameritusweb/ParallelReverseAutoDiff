//------------------------------------------------------------------------------
// <copyright file="MeanAbsoluteErrorLossOperation.cs" author="ameritusweb" date="7/6/2024">
...
namespace ParallelReverseAutoDiff.RMAD.LossOps
{
    ...
    /// <summary>
    /// The Mean absolute error loss operation.
    /// </summary>
    public class MeanAbsoluteErrorLossOperation : PradOperationBase<BinaryCrossEntropyLossOperation, Matrix, Matrix, Matrix>
    {
        // Store the target matrix for use in the backward pass
        private Matrix target;

        /// <summary>
        /// A factory method for creating a mean absolute error loss function.
        /// </summary>
        /// <param name="net">The neural network.</param>
        /// <returns>The instantiated mean absolute error operation.</returns>
        public static IOperation Instantiate(NeuralNetwork net)
        {
            return new MeanAbsoluteErrorLossOperation();
        }

        /// <summary>
        /// The forward pass of the mean absolute error loss function.
        /// </summary>
        ...
        public override Matrix Forward(Matrix output, Matrix target)
        {
            ...
            var mae = PradTools.Zero;

            // Using LINQ to calculate the MAE
            mae = (from row in Enumerable.Range(0, rowCount)
                   from col in Enumerable.Range(0, colCount)
                   let diff = Math.Abs(output[row][col] - target[row][col])
                   select diff).Sum();

            Matrix loss = new Matrix(1, 1);
            loss[0][0] = mae / (rowCount * colCount);
            return loss;
        }
        ...
        public override BackwardResult Backward(Matrix output)
        {
            ...
            for (int row = 0; row < rowCount; row++)
            {
                ...
                {
                    double diff = output[row][col] - this.target[row][col];
                    gradient[row][col] = scale * Math.Sign(diff);
                }
            }
