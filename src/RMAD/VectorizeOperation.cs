...
namespace ParallelReverseAutoDiff.RMAD
{
    ...
    public class VectorizeOperation : Operation
    {
        ...
        public Matrix Forward(Matrix input, Matrix angles)
        {
            ...
            var angleMappings = new Dictionary<(double, double, double), double>();
            ...
            for (int i = 0; i < rows; i++)
            {
                for (int j = 0; j < cols; j++)
                {
                    ...
                    double prev = j > 0 ? input[i][j - 1] : double.MinValue;
                    double next = j < cols - 1 ? input[i][j + 1] : double.MaxValue;
        ...
        public override BackwardResult Backward(Matrix dLdOutput)
        {
            ...
            var angleUsage = new Dictionary<double, List<(int, int)>>(); // Tracks angle usage
