...
namespace ParallelReverseAutoDiff.Test.Common
{
    ...
    {
        private static readonly float MinClipValue = 1E-6f;
        ...
        public static DeepMatrix[][] ClipGradients(DeepMatrix[][] gradients, float clipValue, float? minValue)
        {
        ...
        /// <param name="minValue">The minimum threshold value.</param>
        /// <returns>The clipped gradients.</returns>
        public static Matrix ClipGradients(Matrix gradients, float clipValue, float? minValue)
        {
            if (minValue == null)
            {
                minValue = MinClipValue;
        ...
        public static DeepMatrix StandardizedMatrix(DeepMatrix deepMatrix)
        {
            ...
            foreach (var matrix in deepMatrix)
            {
                foreach (float[] row in matrix)
                {
                    foreach (float value in row)
                    {
                        varianceSum += PradMath.Pow(value - mean, 2);
                    }
                }
            }

            float stdDev = PradMath.Sqrt(varianceSum / count);
        ...
        public static float[] To1DArray(Matrix[] matrices)
        {
            int numMatrices = matrices.Length;
            float[] array = new float[numMatrices];
            for (int i = 0; i < numMatrices; ++i)
            {
        ...
        public static void ClearMatrices(Matrix[] matrices)
        {
            ...
            Parallel.For(0, numMatrices, i =>
            {
                ...
                for (int j = 0; j < numRows; ++j)
                {
                    matrices[i][j] = new float[numCols];
                }
            });
        }
        ...
        public static float FrobeniusNorm(Matrix weightMatrix)
        {
            float sum = 0.0f;
        ...
        public static float LearningRateReductionFactor(float frobeniusNorm, float maxNorm, float minFactor)
        {
            ...
            {
                float reductionFactor = PradMath.Pow(maxNorm / frobeniusNorm, 3f);
        ...
        /// <returns>The resultant matrix.</returns>
        public static Matrix ScalarMultiply(float scalar, Matrix matrix)
        {
        ...
        public static Matrix InitializeRandomMatrixWithXavierInitialization(int numRows, int numCols)
        {
            ...
            Parallel.For(0, numRows, i =>
            {
                for (int j = 0; j < numCols; j++)
                {
                    matrix[i][j] = ((float)Random.NextDouble() * 2 - 1) * PradMath.Sqrt(6.0f / (numRows + numCols));
                }
            });
        ...
        public static Matrix[] InitializeRandomMatrixWithXavierInitialization(int numLayers, int numRows, int numCols)
        {
            ...
            Parallel.For(0, numLayers, layerIndex =>
            {
                ...
                for (int i = 0; i < numRows; i++)
                {
                    for (int j = 0; j < numCols; j++)
                    {
                        matrix[layerIndex][i][j] = ((float)Random.NextDouble() * 2 - 1) * PradMath.Sqrt(6.0f / (numRows + numCols));
                    }
                }
            });
