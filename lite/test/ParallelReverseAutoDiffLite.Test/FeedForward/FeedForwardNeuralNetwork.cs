...
namespace ParallelReverseAutoDiff.Test.FeedForward
{
    ...
    {
        private const string NAMESPACE = "ParallelReverseAutoDiffLite.Test.FeedForward.Architecture";
        ...
        public FeedForwardNeuralNetwork(int inputSize, int hiddenSize, int outputSize, int numLayers, float learningRate, float? clipValue)
        {
        ...
        /// </summary>
        internal float ClipValue { get; private set; } = 4f;

        /// <summary>
        /// Gets the Adam iteration for the neural network.
        /// </summary>
        internal float AdamIteration { get; private set; }
    ...
    public partial class FeedForwardNeuralNetwork
    {
        ...
        private void UpdateWeightWithAdam(Matrix w, Matrix mW, Matrix vW, Matrix gradient, float beta1, float beta2, float epsilon)
        {
            ...
            Matrix mW_hat = CommonMatrixUtils.ScalarMultiply(1f / (1f - PradMath.Pow(beta1, this.AdamIteration)), mW);

            // Compute bias-corrected second raw moment estimate
            Matrix vW_hat = CommonMatrixUtils.ScalarMultiply(1f / (1f - PradMath.Pow(beta2, this.AdamIteration)), vW);
            ...
            for (int i = 0; i < w.Length; i++)
            {
                ...
                {
                    float weightReductionValue = this.Parameters.LearningRate * mW_hat[i][j] / (PradMath.Sqrt(vW_hat[i][j]) + epsilon);
