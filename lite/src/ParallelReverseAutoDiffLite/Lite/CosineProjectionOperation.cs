...
namespace ParallelReverseAutoDiff.RMAD
{
    ...
    public class CosineProjectionOperation : Operation
    {
        ...
        public override BackwardResult Backward(Matrix dOutput)
        {
            ...
            {
                float decimalNumber = this.inputMatrix[0][j];

                for (int i = 0; i < N; i++)
                {
                    // Gradient of the influence function with respect to the decimalNumber
                    float influence = this.CalculateDigitInfluence(decimalNumber, i);
                    float influenceGradient = influence * (1 - influence); // derivative of sigmoid

                    // Calculate the influence of the i-th digit on the output
                    float influenceOnOutput = dOutput[j][i] / (influence / 10) * PradMath.Sqrt(N);

                    // Gradient of the output with respect to the scaling factor
                    float scalingFactor = PradMath.Pow(10, this.learnedScalings[0][i]) / 10;
                    float scalingFactorGradient = -influenceOnOutput / (scalingFactor * scalingFactor);

                    // Adjust the gradient by the steepness of the sigmoid and the derivative of the digit influence calculation
                    dLearnedScalings[0][i] += scalingFactorGradient * PradMath.Log(10) * decimalNumber * PradMath.Pow(10, i) * influenceGradient;
                }
            }
        ...
        private float CalculateDigitInfluence(float decimalNumber, int digitPosition)
        {
            // Shift the decimal number to focus on the current digit
            float shiftedNumber = (decimalNumber * PradMath.Pow(10, digitPosition)) % 10;
            ...
            float steepness = 10; // Example value, adjust based on desired smoothness
            return 1 / (1 + PradMath.Exp(-steepness * (shiftedNumber - 5)));
        }
    }
}
