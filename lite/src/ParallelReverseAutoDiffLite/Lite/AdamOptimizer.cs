...
namespace ParallelReverseAutoDiff.RMAD
{
    ...
    public class AdamOptimizer
    {
        ...
        private void ReverseAdamUpdate(Matrix w, Matrix mW, Matrix vW, Matrix gradient, float beta1, float beta2, float epsilon)
        {
            // Compute bias-corrected first moment estimate
            Matrix mW_hat = MatrixUtils.ScalarMultiply(1 / (1 - PradMath.Pow(beta1, this.network.Parameters.AdamIteration)), mW);

            // Compute bias-corrected second raw moment estimate
            Matrix vW_hat = MatrixUtils.ScalarMultiply(1 / (1 - PradMath.Pow(beta2, this.network.Parameters.AdamIteration)), vW);
            ...
            for (int i = 0; i < w.Length; i++)
            {
                ...
                {
                    float weightReductionValue = this.network.Parameters.LearningRate * mW_hat[i][j] / (PradMath.Sqrt(vW_hat[i][j]) + epsilon);
        ...
        private void UpdateWeightWithAdam(Matrix w, Matrix mW, Matrix vW, Matrix gradient, float beta1, float beta2, float epsilon)
        {
            ...
            // Compute bias-corrected first moment estimate
            Matrix mW_hat = MatrixUtils.ScalarMultiply(1 / (1 - PradMath.Pow(beta1, this.network.Parameters.AdamIteration)), firstMoment);

            // Compute bias-corrected second raw moment estimate
            Matrix vW_hat = MatrixUtils.ScalarMultiply(1 / (1 - PradMath.Pow(beta2, this.network.Parameters.AdamIteration)), secondMoment);
