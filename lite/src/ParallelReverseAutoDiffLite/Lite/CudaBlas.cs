...
namespace ParallelReverseAutoDiff.RMAD
{
    ...
    public class CudaBlas : IDisposable
    {
        ...
        private CudaDeviceVariable<float> dA;

        private CudaDeviceVariable<float> dB;

        private CudaDeviceVariable<float> dC;
        ...
        public void InitializeDeviceVariables(int rows1, int cols1, int rows2, int cols2, bool transposeMatrix1, bool transposeMatrix2)
        {
            ...
            if (!this.AreDeviceVariablesInitialized)
            {
                this.dA = new CudaDeviceVariable<float>(rows1 * cols1);
                this.dB = new CudaDeviceVariable<float>(rows2 * cols2);
                this.dC = new CudaDeviceVariable<float>(c);
            }
            else
            {
        ...
        public Matrix MatrixMultiply(Matrix matrix1, bool transposeMatrix1, Matrix matrix2, bool transposeMatrix2)
        {
            ...
            float[] a_flat = MatrixUtils.FlattenMatrix(matrix1);
            float[] b_flat = MatrixUtils.FlattenMatrix(matrix2);
        ...
        /// <param name="transposeMatrix2">Whether to transpose matrix B before multiplying.</param>
        /// <returns>The resultant matrix.</returns>
        public Matrix MatrixMultiply(float[] matrixFlat1, int rows1, int cols1, bool transposeMatrix1, float[] matrixFlat2, int rows2, int cols2, bool transposeMatrix2)
        {
        ...
        public Matrix WriteMatricesToSharedMemory(Matrix matrixA, bool transposeA, Matrix matrixB, bool transposeB)
        {
            ...
            int bufferSizeA = 1 + (3 * sizeof(int)) + (matrixA.Rows * matrixA.Cols * sizeof(float));
            int bufferSizeB = 1 + (3 * sizeof(int)) + (matrixB.Rows * matrixB.Cols * sizeof(float));
        ...
        public void CudaThreadMethod()
        {
            while (true)
            {
                ...
                int matrixASerializedLength = 1 + (3 * sizeof(int)) + (rowsA * colsA * sizeof(float));
                ...
                if (rowsA > 0 && rowsB > 0 && colsA > 0 && colsB > 0)
                {
                    ...
                    int matrixBSerializedLength = 1 + (3 * sizeof(int)) + (rowsB * colsB * sizeof(float));
                    ...
                    float[] matrixFlatA = Matrix.DeserializeToFlatArray(serializedMatrixA.Span);
                    float[] matrixFlatB = Matrix.DeserializeToFlatArray(serializedMatrixB.Span);
