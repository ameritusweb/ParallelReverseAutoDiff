...
namespace ParallelReverseAutoDiff.RMAD
{
    ...
    public class ElementwiseSquareAndSumOperation : Operation
    {
        ...
        public Matrix Forward(Matrix a, Matrix b)
        {
            ...
            {
                sum1 += PradMath.Pow(a[0][i], 2);
                sum2 += PradMath.Pow(b[0][i], 2);
            }

            this.Output = new Matrix(new float[][] { new float[] { sum1, sum2 } });
            return this.Output;
        }
        ...
        public override BackwardResult Backward(Matrix dOutput)
        {
            ...
            float dOutput1 = dOutput[0][0]; // Gradient for the first sum
            float dOutput2 = dOutput[0][1]; // Gradient for the second sum
