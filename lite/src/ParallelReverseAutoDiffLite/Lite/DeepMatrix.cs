...
namespace ParallelReverseAutoDiff.RMAD
{
    ...
    public class DeepMatrix : IEnumerable<Matrix>, ICloneable
    {
        ...
        public float this[int depth, int row, int col]
        {
        ...
        /// <returns>The resultant deep matrix.</returns>
        public static DeepMatrix operator *(DeepMatrix m, float scalar)
        {
            int depth = m.Depth;
            int numRows = m.Rows;
            int numCols = m.Cols;
        ...
        /// <summary>
        /// Initializes a float array of deep matrices.
        /// </summary>
        ...
        /// <param name="height">The height.</param>
        /// <param name="width">THe width.</param>
        /// <returns>A float array of deep matrices.</returns>
        ...
        public Tensor ToTensor()
        {
            ...
            float[] tensorData = new float[depth * rows * cols];
        ...
        public DeepMatrix Average(DeepMatrix other)
        {
            ...
            Parallel.For(0, this.Depth, d =>
            {
                for (int i = 0; i < this.Rows; ++i)
                {
                    ...
                    {
                        result[d, i, j] = (this[d, i, j] + other[d, i, j]) / 2.0f;
                    }
                }
            });
        ...
        /// </summary>
        /// <param name="rows">The rows.</param>
        public void Replace(List<float[]> rows)
        {
            int skip = 0;
            for (int i = 0; i < this.Depth; ++i)
            {
        ...
        private void InitializeHe(float scalingFactor = 1.0f, float shiftingFactor = 0.0f)
        {
            ...
            Parallel.For(0, this.Depth, d =>
            {
                for (int i = 0; i < this.Rows; ++i)
                {
                    for (int j = 0; j < this.Cols; j++)
                    {
                        this[d, i, j] = (PradMath.Sqrt(variance) * (float)MatrixUtils.Random.NextDouble() * scalingFactor) + shiftingFactor;
                    }
                }
            });
        }
        ...
        private void InitializeXavier(float scalingFactor = 1.0f)
        {
            Parallel.For(0, this.Depth, d =>
            {
                for (int i = 0; i < this.Rows; i++)
                {
                    for (int j = 0; j < this.Cols; j++)
                    {
                        this[d, i, j] = (((float)MatrixUtils.Random.NextDouble() * 2) - 1) * PradMath.Sqrt(6.0f / (this.Rows + this.Cols)) * scalingFactor;
                    }
                }
            });
        }

        private void InitializeXavierUniform(float scalingFactor = 1.0f)
        {
            ...
            for (int i = 0; i < this.Rows; i++)
            {
                for (int j = 0; j < this.Cols; j++)
                {
                    weights[i, j] = (((float)MatrixUtils.Random.NextDouble() * 2) - 1) * PradMath.Sqrt(6.0f / (this.Rows + this.Cols)) * scalingFactor;
                }
            }
