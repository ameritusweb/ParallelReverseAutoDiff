...
namespace ParallelReverseAutoDiff.RMAD
{
    ...
    public class HadamardScaledProductOperation : Operation
    {
        ...
        public Matrix Forward(Matrix input1, Matrix input2, float minValue)
        {
            this.input1 = input1;
            this.input2 = input2;
            ...
            float maxVal = Math.Max(minValue, Math.Max(input1.ToArray().SelectMany(x => x).Max(), input2.ToArray().SelectMany(x => x).Max()));
        ...
        public override BackwardResult Backward(Matrix dLdOutput)
        {
            ...
            float maxVal = Math.Max(this.minValue, Math.Max(this.input1.ToArray().SelectMany(x => x).Max(), this.input2.ToArray().SelectMany(x => x).Max()));
