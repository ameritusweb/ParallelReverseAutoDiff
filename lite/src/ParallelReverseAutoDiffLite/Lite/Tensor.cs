...
namespace ParallelReverseAutoDiff.PRAD
{
    ...
    public partial class Tensor
    {
        ...
        public static Tensor Range(float start, float limit, float delta = 1.0f)
        {
            if (delta == 0)
            {
        ...
        public static Tensor RandomUniform(int[] shape, float minValue = 0.0f, float maxValue = 1.0f)
        {
        ...
        public Tensor Multinomial(int numSamples, float temperature = 1.0f)
        {
            if (this.Shape.Length != 2)
            {
