    ...
    public class VariedMaskedSoftmaxOperation : Operation
    {
        ...
        public Matrix Forward(Matrix input, Matrix temp, Matrix previousSoftmaxOutput, float maskThreshold)
        {
            ...
            for (int i = 0; i < this.input.Cols; i++)
            {
                ...
                {
                    sumExp += PradMath.Exp(this.input[0][i] / this.temperature);
                }
            }
        ...
        public override BackwardResult Backward(Matrix dLdOutput)
        {
            ...
            for (int i = 0; i < numCols; i++)
            {
                ...
                {
                    float expVal = PradMath.Exp(this.input[0][i] / this.temperature);
                    softmax[i] = expVal;
                    sumExp += expVal;
                }
                else
            ...
            float[,] gradSoftmaxTemp = new float[numCols, numCols];
            ...
            Parallel.For(0, numCols, i =>
            {
                if (this.previousSoftmaxOutput[0][i] <= this.maskThreshold)
                {
                    for (int j = 0; j < numCols; j++)
                    {
                        ...
                        gradSoftmaxTemp[i, j] = -this.input[0][j] / PradMath.Pow(this.temperature, 2) * softmaxGrad[i, j];
                    }
                }
            });
            ...
            Parallel.For(0, numCols, i =>
            {
                if (this.previousSoftmaxOutput[0][i] <= this.maskThreshold)
                {
                    float gradientSumX = 0;
                    float gradientSumTemp = 0;

                    for (int j = 0; j < numCols; j++)
                    {
                        float totalGradX = ((softmaxGrad[i, j] / this.temperature) * scaleFactor * dLdOutput[0][j]) +
                                            (softmax[i] * scaleFactorGradient * dLdOutput[0][j]);
                        gradientSumX += totalGradX;

                        float totalGradTemp = (gradSoftmaxTemp[i, j] * scaleFactor * dLdOutput[0][j]) +
                                               (softmax[i] * scaleFactorGradient * dLdOutput[0][j]);
